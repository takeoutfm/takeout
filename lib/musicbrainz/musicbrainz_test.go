...
func TestSearchReleases(t *testing.T) {
	...
	m := NewMusicBrainz(config)
	m.SearchArtistID(arid)
}
...
func TestArtistReleases(t *testing.T) {
	...
	arid := "66fc5bf8-daa4-4241-b378-9bc9077939d2"
	result, err := m.ArtistReleases("", arid)
	if err != nil {
		t.Errorf("bummer %s\n", err)
	}
...
func TestReleaseGroupLookup(t *testing.T) {
	...
	// result, err := m.SearchReleaseGroup("72375978-a9a1-4254-b957-85565c716b7e", "")
	// if err != nil {
	// 	t.Errorf("bummer %s\n", err)
	// }
	// //fmt.Printf("got %+v\n", result)
...
func TestReleaseLookup(t *testing.T) {
	...
	result, err := m.Release("ad60fee6-a25b-4d52-8a9f-00c5fa508c34")
	if err != nil {
		t.Errorf("bummer %s\n", err)
	}
	fmt.Printf("\n")
...
func TestSearchArtist(t *testing.T) {
	...
	m := NewMusicBrainz(config)
	a := m.SearchArtist("Daniel Ash")
	t.Logf("%s, %s, %s", a.Name, a.SortName, a.ID)
}
...
func TestSearchArtistId(t *testing.T) {
	...
	m := NewMusicBrainz(config)
	a := m.SearchArtistID("0383dadf-2a4e-4d10-a46a-e9e041da8eb3")
	t.Logf("%s, %s, %s", a.Name, a.SortName, a.ID)
}
