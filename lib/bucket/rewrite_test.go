...
func TestRewrite(t *testing.T) {
	...
	if rewrite(rules, "/bucket/Unchanged Artist/Unchanged Album (2022)/Unchanged Song.flac") !=
		"/bucket/Unchanged Artist/Unchanged Album (2022)/Unchanged Song.flac" {
		t.Error("expect unchanged")
	}
	if rewrite(rules, "/bucket/Music/Dr. Octagon/Dr. Octagon, Part II (2004)/1-Song.flac") !=
		"/bucket/Music/Kool Keith/Dr. Octagon, Part II (2004)/1-Song.flac" {
		t.Error("expect kool keith")
	}
	if rewrite(rules, "/bucket/Music/White Zombie/La Sexorcisto_ Devil Music, Volume One (1992)/1-Dragula.flac") !=
		"/bucket/Music/White Zombie/La Sexorcisto_ Devil Music, Vol. 1 (1992)/1-Dragula.flac" {
		t.Error("expect vol 1")
	}
	if rewrite(rules, "/bucket/Music/Gary Numan/Premier Hits (1996)/09-Foo.flac") !=
...
func TestRewrite2(t *testing.T) {
	...
	if rewrite(rules, "/bucket/Music/Artist/Album/1-Track.flac") !=
